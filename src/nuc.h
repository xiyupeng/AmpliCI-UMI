 ...
 * @param enc1	first nucleotide encoding
 * @param nuc2	second nucleotide
 * @param enc2	second nucleotide encoding
 */
inline int nuc_cmp(data_t nuc1, int enc1, data_t nuc2, int enc2)
{
	if (enc1 == IUPAC_ENCODING && enc2 == IUPAC_ENCODING)
		return !(nuc1 & nuc2);
	else if (enc1 == XY_ENCODING)
		return !(xy_to_iupac[nuc1] & nuc2);
	else if (enc2 == XY_ENCODING)
		return !(xy_to_iupac[nuc2] & nuc1);
	else
		return !(xy_to_iupac[nuc1] & xy_to_iupac[nuc1]);
}/* nuc_cmp */
...
inline void fwrite_xy_sequence(FILE *fp, sequence *s, size_t sidx, size_t eidx)
{
	for (size_t i = sidx; i < eidx; ++i)
		fprintf(fp, "%c", xy_to_char[read_sequence_char(s,
					&_xy_sequence_opt, i)]);
} /* fwrite_xy_sequence */
...
{
	for (size_t i = sidx; i < eidx; ++i)
		fprintf(fp, "%c", iupac_to_char[read_sequence_char(s,
...
{
	return read_sequence_char(s, nuc_sequence_opt(encoding), i);
} /* get_nuc */
