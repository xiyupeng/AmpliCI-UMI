...
{
	return exp(- read_sequence_char(s, &_qual_sequence_opt, i)
							 / 10. * log(10.));
} /* qual_to_prob */
...
{
	return - read_sequence_char(s, &_qual_sequence_opt, i) * log(10.) / 10.;
} /* qual_to_lprob */
...
{
	return read_sequence_char(s, &_qual_sequence_opt, i);
} /* get_qual */

inline char qual_to_char(sequence *s, size_t i)
{
	return (char) read_sequence_char(s, &_qual_sequence_opt, i)
...
inline void fwrite_qual_sequence(FILE *fp, sequence *s)
{
	for (unsigned int i = 0; i < s->len; ++i)
		fprintf(fp, "%c", (char) (read_sequence_char(s,
			&_qual_sequence_opt, i) + MIN_ASCII_QUALITY_SCORE));
