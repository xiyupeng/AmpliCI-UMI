...
int ampliCI_wpartition(options * opt, data * dat, model *mod,
						 initializer *ini, run_info *ri)
{
	...
	for (unsigned int p = 0; p < num_partition; p++) {
	   ...
	   //mmessage(INFO_MSG, NO_ERROR, "start the %d th subset\n",p);
		...
		err = amplici_core(dmat, qmat, (size_t) size, opt->low_bound,
			dat->max_read_length, (char*) opt->error_profile_name,
			dat->fdata->max_quality, dat->fdata->min_quality, 
			&seeds, &seeds_length, &cluster_id, &cluster_size,
							 &K, &ll, &abun);
	...
	if (!opt->outfile_fasta) {
		...
		outfile_hap = malloc((strlen(opt->outfile_base) + 5)
								* sizeof(char));
		if (!outfile_hap)
			return mmessage(ERROR_MSG, MEMORY_ALLOCATION,
								"output file");
	...
	if (!fp2)
		return mmessage(ERROR_MSG, FILE_OPEN_ERROR, opt->outfile_fasta);
		
	fprint_haplotypes_abun(fp2,ini->seeds[0], opt->K, dat->max_read_length,
				opt->p_threshold, "H", NULL, NULL, NULL);
 ...
 * @return		error status
 */
int read_partition_file(char const * const filename, unsigned int *cluster_id,
...
/**
 * Make partition ids contiguous.
 *
 * @param cluster_id	array for storing cluster ids
 * @param sample_size	sample size (length of cluster_id)
 * @return		error status
 */
int make_partition_ids_contiguous(unsigned int *cluster_id,
						unsigned int sample_size)
{
	unsigned int n_id = 0, n_id_used = 0;

	for (unsigned int i = 0; i < sample_size; ++i)
		if (n_id < cluster_id[i])
			n_id = cluster_id[i];

	++n_id;

	unsigned char *cluster_id_used = calloc(n_id,
						sizeof(*cluster_id_used));
	...
	for (unsigned int i = 0; i < sample_size; ++i)
		cluster_id_used[cluster_id[i]] = 1;
	
	for (unsigned int k = 0; k < n_id; ++k)
		if (cluster_id_used[k])
			cluster_id_used[k] = n_id_used++;

	if (n_id == n_id_used) {
		free(cluster_id_used);
		return NO_ERROR;
	}

	for (unsigned int i = 0; i < sample_size; ++i)
		cluster_id[i] = cluster_id_used[cluster_id[i]];
	
	free(cluster_id_used);

	return NO_ERROR;
} /* make_partition_ids_contiguous */
